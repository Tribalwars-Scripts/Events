...
!function () {
	...
	Simulator={
		...
		prefixes: {attacker: "att", defender: "def"},
		$form: null,
		benefits: {attacker: [], defender: []},
		init: function () {
			var e=this.$form=$("#simulator_form");
			e.submit(function (t) {
				e.find("input[name=att_benefits]").val(Simulator.formatBenefitsForTransmission(Simulator.benefits.attacker)), e.find("input[name=def_benefits]").val(Simulator.formatBenefitsForTransmission(Simulator.benefits.defender))
			}), $(".btn-benefit-create").on("click", function (e) {
				e.preventDefault(), Simulator.BenefitCreator.open($(this).data("side"))
			}), this.Filler.init(), this.EffectTemplates.init(), $("#catapult_wall").change(function () {
		...
		addBenefit: function (e, t) {
			...
			$("#" + this.prefixes[t] + "_benefits_display").append(i), this.benefits[t].push(e)
		},
		removeBenefit: function (e, t) {
			$("#" + this.prefixes[e] + "_benefits_display").find("li:nth-child(" + (t + 1) + ")").detach(), this.benefits[e].splice(t, 1)
		},
		removeAllBenefits: function (e) {
			for (; this.benefits[e].length > 0;) this.removeBenefit(e, 0)
		},
		...
		Filler: {
			templates: {
				attacker: {current_village: null, last_sim: null},
				defender: {current_village: null, last_sim: null, sim_survivors: null}
			}, init: function () {
				var e=this;
				$("#att_fill_select").change(function (t) {
					...
					t.preventDefault(), e.fillSide(Simulator.SIDE_ATTACKER, e.templates.attacker.reset), $("#att_fill_select").val("")
				}), $("#reset_def_units").on("click", function (t) {
					t.preventDefault(), e.fillSide(Simulator.SIDE_DEFENDER, e.templates.defender.reset), $("#def_fill_select").val(""), $("#wall_id").val(""), $("#is_rune").prop("checked", !1), $("#is_night").prop("checked", !1)
				})
				...
				t.fill.flag && this.getInputByName(i + "_flag").val(t.flag_level), t.fill.benefits && (Simulator.removeAllBenefits(e), t.benefits.forEach(function (t) {
					Simulator.addBenefit(t, e)
				}), Simulator.EffectTemplates.addBenefitsFromSelectedTemplate(e))
			}, fillUnits: function (e, t) {
				var i=this, a=Simulator.prefixes[e];
		...
		MoraleCalculator: {
			...
			open: function () {
				var e=this, t=$("input[name=morale_type]:checked").val();
				return Dialog.fetch("morale_calculator", "place", {ajax: "morale_calculator", type: t}, function () {
					...
					i.find(".menu-item").click(function () {
						...
						i.preventDefault(), e.calculateMorale(t)
					})
				}, {}, function () {
					e.perspective=null
				}), !1
			...
			calculateMorale: function (e) {
				...
				t.push({name: "perspective", value: this.perspective});
				var i=this;
				TribalWars.post("place", {ajax: "calculate_morale", type: e}, t, function (e) {
					i.setAttackerName(""), i.setAttackerPoints(e.attacker_points), i.setDefenderName(""), i.setDefenderPoints(e.defender_points), i.setDaysPlayed(e.days_played), i.setMorale(e.morale)
				})
			},
			...
			switchPerspective: function (e) {
				if (e !== this.perspective) {
					...
					var i=$("#morale_calculator");
					i.find("form." + t).hide(), i.find(".menu-item[data-perspective=" + e + "]").addClass("selected"), i.find(".menu-item[data-perspective=" + t + "]").removeClass("selected"), i.find(".result_display").hide()
				}
			},
			pasteResultToSimulator: function () {
				Simulator.setMorale(this.calculated_morale), Dialog.close()
			}
		},
		BenefitCreator: {
			possibilities: {attacker: [], defender: []},
			...
			open: function (e) {
				...
				t+='<div class="center"><a href="#" class="btn btn-default">' + i + "</a></div></form>", Dialog.show("benefit_creator", t);
				var a=this, l=$("#benefit_creator_form");
				l.find('select[name="type"]').change(function () {
					a.handleTypeSwitch()
				}), l.find(".btn").click(function (e) {
					e.preventDefault(), a.finishBenefit()
				}), this.handleTypeSwitch()
			},
			handleTypeSwitch: function () {
				var e=$("#benefit_creator_form"), t=e.find('select[name="type"]').find("option:selected");
				e.find(".variable-inputs").html(this.possibilities[t.data("side")][t.data("index")].generateUI())
			},
			finishBenefit: function () {
				TribalWars.post("place", {ajax: "create_benefit"}, $("#benefit_creator_form").serializeArray(), function (e) {
					Simulator.addBenefit(e.benefit, e.side), Dialog.close()
				})
			},
			PossibleBenefit: {
		...
		EffectTemplates: {
			benefits: {attacker: [], defender: []}, init: function () {
				$("#attack_effect_templates").on("change", function (e) {
					if (this.value < 0) return Simulator.removeAllBenefits(Simulator.SIDE_ATTACKER), $("#del_att_temp").prop("disabled", !0), $('input[name="att_template_name"]').val(""), void Simulator.Filler.addBenefitsFromSelectedVillage(Simulator.SIDE_ATTACKER);
					e.preventDefault();
					var t=Simulator.EffectTemplates.benefits.attacker[this.value];
					Simulator.removeAllBenefits(Simulator.SIDE_ATTACKER), Simulator.addBenefits(t.benefits, Simulator.SIDE_ATTACKER), Simulator.Filler.addBenefitsFromSelectedVillage(Simulator.SIDE_ATTACKER), $('input[name="att_template_name"]').val(t.name), $("#del_att_temp").prop("disabled", !1).attr("href", TribalWars.buildURL("GET", {
					...
					if (this.value < 0) return Simulator.removeAllBenefits(Simulator.SIDE_DEFENDER), $("#del_def_temp").prop("disabled", !0), $('input[name="def_template_name"]').val(""), void Simulator.Filler.addBenefitsFromSelectedVillage(Simulator.SIDE_DEFENDER);
					e.preventDefault();
					var t=Simulator.EffectTemplates.benefits.defender[this.value];
					Simulator.removeAllBenefits(Simulator.SIDE_DEFENDER), Simulator.addBenefits(t.benefits, Simulator.SIDE_DEFENDER), Simulator.Filler.addBenefitsFromSelectedVillage(Simulator.SIDE_DEFENDER), $('input[name="def_template_name"]').val(t.name), $("#del_def_temp").prop("disabled", !1).attr("href", TribalWars.buildURL("GET", {
						action: "delete_effect_template",
						screen: "place",
						mode: "sim",
						id: t.id
				...
				if (e == Simulator.SIDE_ATTACKER) var t=$("#attack_effect_templates"); else t=$("#defense_effect_templates");
				var i=$(t).val();
				$(t).find(":selected").remove(), delete Simulator.EffectTemplates.benefits[e][i];
				var a={
					text: _("f2a6c498fb90ee345d997f888fce3b18"), confirm: !0, callback: function () {
						TribalWars.post("place", {ajax: "delete_effect_template"}, {id: i}, function (e) {
				...
				var t=this.sideFromType(e.type);
				TribalWars.post("place", {ajax: "add_effect_template"}, e, function (i) {
					switch (parseInt(i.status)) {
						case 1:
							if (UI.SuccessMessage(i.message), Simulator.EffectTemplates.addTemplate(i.template), 0 == $('.template-list>option[value="' + i.template.id + '"]').length) {
								if (t == Simulator.SIDE_ATTACKER) var a=$("#attack_effect_templates"); else a=$("#defense_effect_templates");
								$('<option value="' + i.template.id + '">' + i.template.name + "</option>").appendTo(a).prop("selected", !0), $(a).trigger("change")
							}
							break;
						case 0:
						...
						case-1:
							...
							l.confirm= !0;
							var n={
								text: _("da364eb37e143f6b2b5559aa03f5913a"), confirm: !0, callback: function () {
