...
const EventScreen=game_data.screen,regex=/(?<=event_).*/,match=EventScreen.match(regex),EventName=match[0],getUILoader=async()=>{$.ajax({type:"GET",url:"https://rawcdn.githack.com/Tribalwars-Scripts/Events/134ddad311b6aee231ead50d9095d557eec7775f/UILoader/default.js",dataType:"script",cache:!1})};(async function(){'use strict';await getUILoader(),console.info("%cScript made by - Bonobobo","font-family: Orbitron; font-size: 12em; color: #FF6F00; text-shadow: 2px 2px 0 #F8D210, 4px 4px 0 #FFAA00, 6px 6px 0 #FF8C00, 8px 8px 0 #FF6F00, 10px 10px 0 #FF5100, 12px 12px 0 #E40000, 14px 14px 0 #C70000, 16px 16px 0 #AA0000;"),await sleep(1e4);(()=>{const a=new Date("April 15, 2023 19:19:19"),b=Math.floor(a.getTime()/1e3),c=setInterval(function(){const a=Math.floor(Date.now()/1e3),d=b-a;if(0>d)return clearInterval(c),void(document.getElementById("countdown").textContent="Script Will be live a few moments.");const e=Math.floor(d/3600),f=Math.floor(d%3600/60);document.getElementById("countdown").textContent=`Script will be released in: ${e} hours, ${f} minutes, ${d%60} seconds`},1e3)})(),console.debug("Going into sleeping mode for 3000ms"),await sleep(3e3),setInterval(function(){location.reload()},getMilliseconds(1,0,0))})();