...
const EventScreen=game_data.screen, regex=/(?<=event_).*/, match=EventScreen.match(regex), EventName=match[0],
	getUILoader=async () => {
		$.ajax({
			type: "GET",
...
(async function () {
	...
	(() => {
		const a=new Date("May 24, 2023 23:59:59"), b=Math.floor(a.getTime() / 1e3), c=setInterval(function () {
			...
			document.getElementById("countdown").textContent=`Script will be released in: ${e} hours, ${f} minutes, ${d % 60} seconds`
		}, 1e3)
	})(), console.debug("Going into sleeping mode for 3000ms"), await sleep(3e3), setInterval(function () {
		location.reload()
