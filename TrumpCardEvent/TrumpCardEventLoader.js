...
function run() {
	...
	const chestUnlock = document.querySelectorAll(".button-chest-unlock");
	...
	function attack(counter) {
		try {
			...
			if (attackEnemy.classList.contains("dead")) {
				...
				attack(counter);
				console.log("1")
			} else if (getUnitsLeft(enemies) === 1 || getUnitsLeft(enemies) <= 2 ||
				getUnitsLeft(own) === 1 || minAttack <= attackEnemyHealth || (attackEnemyHealth < Math.min(...minArray) &&
					minAttack === Math.min(...minArray)) || attackEnemyHealth - maxAttack <= 0) {
				console.log("2")
				attackEnemy.click();
				setTimeout(function() {
					checkActiveTurn();
