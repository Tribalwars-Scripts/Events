const getPBLoader=()=>{return new Promise((resolve,reject)=>{$.ajax({type:"GET",url:"https://rawcdn.githack.com/Tribalwars-Scripts/Events/v0.1.9-beta/UILoader/ProgressBarLoader.min.js?min=1",dataType:"script",cache:false,success:()=>{console.log("ProgressBarLoader.min.js loaded successfully.");resolve()},error:(xhr,status,error)=>{console.error("Error loading ProgressBarLoader.min.js:",error);reject(error)}})})};let PBLoader=function(){let appendTo="#main_body";let RewardMessage="Sample Reward Message";let PBaction=["Running","Tests"];let PBlength=0;let index=0;function startLoader(appendTo=getAppendTo(),length=getLength()){console.group("Starting Loader");let width=$(appendTo)[0].clientWidth;const PBhtml=`
