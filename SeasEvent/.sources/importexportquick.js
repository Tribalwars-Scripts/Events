// ==UserScript==
// @name         Import/Export QuickBar
// @namespace
// @version      0.2
// @description  Import/Export Quickbar, sitting mode fixed.
// @author       fmthemaster
// @discord      fmthemaster#7485
// @fixedby      Bruno Preto
// @include **&screen=settings&mode=quickbar**
...
// var scriptTag = "fmTag";
var forumLink = "https://forum.tribalwars.net/index.php?threads/import-export-quickbar.286825/";
...
function main()
{
	...
	{
		window.location.href = window.location.pathname+ "?screen=settings&mode=quickbar";
		return;
	}
	htmlGenerate();
}
...
function htmlGenerate(){
	...
	let tHead = $("#content_value > table > tbody > tr > td:nth-child(2) > table > thead > tr")[0];
	tHead.insertCell(-1).outerHTML = "<th><input type='checkbox' id='quickbar_box_all' class='checkbox'></th>";
	$("#quickbar > tr").each((key, obj)=>obj.insertCell(-1).innerHTML =`<input type="checkbox" class="checkbox quickbar_box">`);
...
async function exportQuickBar() {
	$("#importExportInput")[0].style.display = "initial";
	$("#importExportInput")[0].value = encrypt(JSON.stringify(await getScripts()));
}

async function exportQuickBarToFile() {
	createFile(encrypt(JSON.stringify(await getScripts())), 'quickBar.txt', 'text/plain');
}
...
async function getScripts(){
	...
	toExport = $.grep(toExport, (obj)=> $(".quickbar_box", obj.parentNode.parentNode)[0].checked);
	let toExportLength = toExport.length;
	startLoader(toExportLength);
	let scriptList = await Object.assign({}, ...await Promise.all($(toExport).map(async (key, obj)=>{return{[key]: await new Promise((resolve)=>{setTimeout(async ()=>{
			$.ajax(obj.href, {
				success:(data)=> {
					let formval = $("form", data).serialize().replace(/hash=([^&]*)\&/, "").replace(/\&h=([^&]*)/, "");
	...
	let structure = $("tr[id^=quickbar] > td:nth-child(1)").map((key, obj)=> !$("hr", obj).length).toArray();
...
function setScript(formval)
{
	...
	// Fixes the bug where the URL contains &t=keyval
	let action = game_data.link_base_pure + "settings&mode=quickbar_edit&action=quickbar_edit&";
...
async function deleteSelected(){
	...
	toDelete = $.grep(toDelete, (obj)=> $(".quickbar_box", obj.parentNode.parentNode)[0].checked);
	let toDeleteLength = toDelete.length;
	startLoader(toDeleteLength);
	await Promise.all($.map(toDelete, async (obj, key)=>{console.log(key,obj);
		return new Promise((resolve)=> setTimeout(()=> $.ajax({url:obj.href, type:"GET", success:()=>{loaded(key, toDeleteLength, "Deleting"); return resolve();}, error:()=>resolve()}), 250*key));}));
}

async function createQuickBar(quickbar){
	if($("#overwrite_checkbox")[0].checked)
	...
	let i=0;
	$.map(quickbar.structure, (obj, key)=>setTimeout(()=>{
		loaded(key, quickBarLength, "Importing");
		if(obj)
			setScript(quickbar.scriptList[i++].formval);
		else
			$.ajax({url:linebreak, type:"GET"});
	}, 250*key));
}
